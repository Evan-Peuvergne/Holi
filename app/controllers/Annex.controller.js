	...
	var Annex = function (doc, callbacks)
	{
		...
		this.initSlide = function (index)
		{
			...
			image.onload = function ()
			{
				...
				// Manage next slide
				if(index < that.slider.total - 1){ console.log('return back'); that.initSlide(index+1); }
		...
		this.previous = function ()
		{
			...
			if(this.slider.current > 0)
			{
				...
				// Update button state
				if(this.slider.current > 0){ console.log('previous enabled'); this.dom.annex.previous.addClass('enabled').removeClass('disabled'); }else{ console.log('previous disabled'); this.dom.annex.previous.addClass('disabled').removeClass('enabled'); }
				if(this.slider.current < this.slider.total - 1){ console.log('next enabled'); this.dom.annex.next.addClass('enabled').removeClass('disabled'); }else{ console.log('next disabled'); this.dom.annex.next.addClass('disabled').removeClass('enabled'); }
			}
		...
		this.next = function ()
		{
			...
			if(this.slider.current + 1 < this.slider.total)
			{
				...
				// Update button state
				if(this.slider.current > 0){ console.log('previous enabled'); this.dom.annex.previous.addClass('enabled').removeClass('disabled'); }else{ console.log('previous disabled'); his.dom.annex.previous.addClass('disabled'); }
				if(this.slider.current < this.slider.total - 1){ console.log('next enabled'); this.dom.annex.next.removeClass('disabled'); }else{ console.log('next disabled'); this.dom.annex.next.addClass('disabled').removeClass('enabled'); }
